...
Hadoop Metrics2 framework is designed to collect and dispatch per-process metrics to monitor the overall status of the Hadoop system. 
Metrics sources are where the metrics are generated and metrics sinks consume the records generated by the metrics sources. 
A metrics system would poll the metrics sources periodically and pass the metrics records to metrics sinks.

* Hadoop has implemented 2 sink plugins: fileSink and gangliaSink
* Ambari has implemented 1 sink plugin: ambari-metrics-hadoop-sink

# Hadoop metrics elasticsearcg sink plugin #

hadoop-metrics-elasticsearch-sink is an implementation of sink plugin to push metrics to Elasticsrarch (a distributed RESTful search engine).
The sink is capable of collecting metrics of hadoop applications that support Hadopp Metrics2 (e.g. hdfs, yarn, hbase, etc.).
...
# Prepar slider app package for hbase #

* Build slider(0.80.0) app packge for hbase (1.1.1)
* Unzip the package and change following configuration in file: "$PACKAGENAME/package/templates/hadoop-metrics2-hbase.properties-GANGLIA-MASTER.j2"
```
...
hbase.sink.elasticsearch.componentName-prefix=HBASE_MASTER
{% else %}
```

* Change following configuration in file: "$PACKAGENAME/package/templates/hadoop-metrics2-hbase.properties-GANGLIA-RS.j2"
```
{% if has_metric_collector %}
*.elasticsearch.plugin.urls={{metric_collector_lib}}
hbase.class=org.apache.hadoop.metrics2.sink.elasticsearch.ElasticsearchMetricsSink
...
hbase.sink.elasticsearch.appName-prefix={{app_name}}
hbase.sink.elasticsearch.componentName-prefix=HBASE_REGIONSERVER
{% else %}
```

* Change following configuration in file: "$PACKAGENAME/appConfig-default.json", or dynamically set in Ambari Slider View when deploying hbase.
Make sure the location of your sink.jar is acceable by all data nodes.
```
"site.global.metric_collector_host": "${YOUR_ELASTICSERCH_HOST}",
"site.global.metric_collector_port": "9200",
"site.global.metric_collector_lib": "${YOUR_SINK_PATH_JAR}",
```

* Create the zip package, and depoly hbase through slider. All hbase emitted metrics would be pushed to elasticsearch.
